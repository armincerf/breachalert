# breachalert Deployment

- [breachalert Deployment](#bedrock-deployment)
  - [Setup](#setup)
...
- Use `gcloud config configurations create breachalert` and authorize Google Cloud `gcloud auth login`
...
```bash
gcloud projects create breachalert-staging --name="breachalert Staging"
...
```bash
./deployment/scripts/provision_gcloud staging breachalert-staging
```
