...
![Run Tests](https://github.com/armincerf/breachalert/workflows/Tests/badge.svg)
...
- `MONGO_URI` - MongoDB URI to connect to, defaults to `mongodb://localhost/breachalert_dev`
...
- `ADMIN_EMAIL` - Default dashboard admin user `alexdavis@hey.com`
- `ADMIN_PASSWORD` - Default dashboard admin password `[change me]`
- `APP_NAME` - Default product name to be used in emails `breachalert`
- `APP_URL` - URL for app defaults to `http://localhost:2200`
- `POSTMARK_FROM` - Reply email address `no-reply@alexthings.co.uk`
...
```
docker build -t breachalert-api .
```
...
```
docker build -t breachalert-api-jobs -f Dockerfile.jobs .
```
